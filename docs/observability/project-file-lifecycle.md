...
`file_links`) and the storage it sits on. Requirement R-NFR-5 / AC-31: presign,
finalize and delete emit structured log records carrying the workspace, project, file
and version identifiers plus an outcome, and four counters exist for the failures
...
| `file.delete`   | `purge_file()`, after the commit — the `DELETE …/purge/` endpoint and the scheduled retention task share it | `purged`, `purge_failed` (with `trigger`, `object_key`)                       |
...
- The counters are **not transactional**. Each is incremented once the row change it
  describes has committed, so a crash in between loses the increment — the audit row
  (`file_access_logs`) is the record of truth for a single event. Use the counters for
  rates and alerting, the audit trail for evidence.
...
signing, and the list benchmark by removing the seed's `ANALYZE` (its p95 then reads
~3.7 s instead of ~0.03 s — a planner-statistics effect of the seed, not of the endpoint).
